...
function age(arr) {
	...
	for (let i = 0; i < arr.length; i++) {
		...
		// cortei um dobrado pra entender que se não colocasse: (arr.length - 1) ou (i < arr.length [no lugar de <=])
		//na última iteração do array o i seria indefinido e por isso ficaria dando um erro e não permitia que eu desse
		//um console.log fora do for.e por causa desse erro eu era obrigado a dar um console.log dentro do for ou do if
		//o que gerava duplicidade de informações.Descobri esse erro utilizando o comando ? para verificar se a variável
		//que está sendo acessada é undefine ou nula.Então coloquei um console.log(arr[i]?.age) dentro do for e percebi
		//que na última iteração do array ele ficava undefined.E aqui precisa ser arr.length - 1 pq a cada iteração está
		//sendo feita uma comparação entre o item atual com o próximo.nesse caso não precisa ir até o último item do array
		//pq  o próximo será undefined.
	...
	console.log(
		"A maior idade é de: " +
			oldestOne[oldestOne.length - 1].name +
			" com " +
			oldestOne[oldestOne.length - 1].age +
			" anos."
	);
}
...
function gender() {
	...
	console.log(
		"existe " + howManyWomansGreaterThan20Years + " mulheres acima de 20 anos"
	);
}
...
function infoOfPeoples(arr) {
	...
	for (let i = 0; i < arr.length; i++) {
		...
		if (arr[i].weight < 50 && arr[i].height < 1.6) {
			pesamMenosQue50 += 1;
		}
		// medem mais de 1.90 e pesam mais de 100kg
		if (arr[i].height > 1.9 && arr[i].weight > 100) {
			medemMaisQue90 += 1;
		}
	}
	...
	console.log(
		pesamMenosQue50 + " pessoas pesam menos de 50kg e tem menos de 1,60m"
	);
	console.log(
		medemMaisQue90 + " pessoas medem mais de 1.90 e pesam mais de 100kg"
	);
}
